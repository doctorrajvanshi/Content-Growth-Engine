...
Given a post URL or raw text, draft a reply linking to the most relevant local
guide. Twitter fetch (optional) uses config/credentials.json 'twitter_bearer'.
...
def get_tweet(tweet_id: str) -> dict | None:
    tok = cfg.get("twitter_bearer")
    if not tok:
        return None
    try:
        import urllib.request
        url = (f"https://api.twitter.com/2/tweets/{tweet_id}"
               f"?tweet.fields=text,author_id&expansions=author_id&user.fields=username")
...
def draft(text: str, platform: str) -> str:
    ...
    if hits:
        ...
        insight = " ".join(re.split(r"(?<=[.!?])\s+", re.sub(r"[#*_`]", "", body))[2:4])[:180]
